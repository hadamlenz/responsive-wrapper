...
export default class ResponsiveWrapperInit {
  constructor(options) {
    ...
    //console.log( options.possibleWrapperArray );
    if (options.possibleWrapperArray) {
      possibleWrappers = [...possibleWrappers, ...options.possibleWrapperArray];
    }
    ...
    for (var i in possibleWrappers) {
      ...
      if (
        !possibleWrappers[i].hasChildNodes() &&
        !possibleWrappers[i].hasAttribute("data-rw-group")
      ) {
      ...
      if (i == 0) {
        ...
        if (
          singleWrappers[i - 1] &&
          singleWrappers[i - 1].contains(possibleWrappers[i])
        ) {
    ...
    for (var i in singleWrappers) {
      ...
      if (singleWrappers[i] && singleWrappers[i] !== null) {
        ...
        let singleOpions = [];
        singleOpions.iframe = this.iframe ? this.iframe : null;
        new ResponsiveWrapperSingle(singleWrappers[i], null, singleOpions);
      }
    }
